...
public class Frame_DeptController {
    ...
    @ResponseBody
    @RequestMapping(value = "/updateDept",produces="application/json;charset=utf-8",method= RequestMethod.PUT)
    public R updateDept(@RequestBody Frame_Dept frameDept){
    	Date updateTime = DateUtil.changeDate(new Date());
    	frameDept.setUpdateTime(updateTime);
    ...
    public R deleteDept(@RequestBody String[] guids){
    	...
    	for (String guid : guids) {
    		...
    		List<Frame_Dept> depts = deptService.isHasChild(guid);
    		if (depts == null || depts.size() == 0) {
    			 deptService.deleteFrameDeptById(guids);
			}else{
				return R.error("该部门下仍有子部门，清先删除子部门");
			}
		}
        return R.ok();
    }
